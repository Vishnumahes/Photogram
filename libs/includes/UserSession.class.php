...
class UserSession
{
    ...
    public static function authenticate($user, $pass)
    {
        ...
        if ($username and isset($_COOKIE['visitorId'])) {
            ...
            $sql = "INSERT INTO `session` (`uid`, `token`, `login_time`, `ip`, `user_agent`, `active`, `visitorId`)
            VALUES ('$user->id', '$token', now(), '$ip', '$agent', '1', '$visitorId')";
            if ($conn->query($sql)) {
    ...
    /*
    * Authorize function have has 5 level of checks 
        ...
        5.Check that the current browser visitorId same as the previous visitorId
    ...
    */
                ...
                if ($session->isValid() and $session->isActive()) {
                    if ($_SERVER['REMOTE_ADDR'] == $session->getIP()) {
                        if ($_SERVER['HTTP_USER_AGENT'] == $session->getUserAgent()) {
                            if ($_COOKIE['visitorId'] == $session->getVisitorId()) {
                                return true;
                            } else throw new Exception("Browser fingerprint does't match");
    ...
    {
        return isset($this->data['visitorId']) ? $this->data['visitorId'] : false;
    }
